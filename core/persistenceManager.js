...
export async function snapshotStateToMessage(messageId, recordTs) {
    ...
    // Force the deterministic turn time if provided. El snapshot DEBE reflejar
    // la hora exacta del turno aunque la captura ocurra después (p.ej. aceptar
    // o rechazar un [UPDATE_STATE] con el reloj ya adelantado al turno +1).
    if (recordTs !== undefined && recordTs !== null) {
        if (snapshot.time) snapshot.time = { timestamp: recordTs };
    }
    
    message.extra.stateTrackerSnapshot = snapshot;
